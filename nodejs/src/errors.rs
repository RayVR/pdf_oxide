...
pub fn map_error(err: PdfError) -> NapiError {
  let (code, message) = match err {
    PdfError::InvalidHeader(header) => {
      ("INVALID_HEADER", format!("Invalid PDF header: expected '%PDF-', found '{}'", header))
    }
    ...
    PdfError::ParseWarning { offset, message: msg } => {
      ("PARSE_WARNING", format!("Parse warning at byte {}: {}", offset, msg))
    }
    PdfError::InvalidXref => {
      ("INVALID_XREF", "Invalid cross-reference table".to_string())
    }
    ...
    PdfError::InvalidObjectType { expected, found } => {
      ("INVALID_OBJECT_TYPE", format!("Invalid object type: expected {}, found {}", expected, found))
    }
    PdfError::UnexpectedEof => {
      ("UNEXPECTED_EOF", "End of file reached unexpectedly".to_string())
    }
    PdfError::Io(io_err) => {
      ("IO_ERROR", format!("IO error: {}", io_err))
    }
    ...
    PdfError::InvalidPdf(reason) => {
      ("INVALID_PDF", format!("Invalid PDF: {}", reason))
    }
    PdfError::Decode(reason) => {
      ("DECODE_ERROR", format!("Stream decoding error: {}", reason))
    }
    PdfError::Encode(reason) => {
      ("ENCODE_ERROR", format!("Encoding error: {}", reason))
    }
    PdfError::UnsupportedFilter(filter) => {
      ("UNSUPPORTED_FILTER", format!("Unsupported filter: {}", filter))
    }
    PdfError::Font(reason) => {
      ("FONT_ERROR", format!("Font error: {}", reason))
    }
    PdfError::Image(reason) => {
      ("IMAGE_ERROR", format!("Image error: {}", reason))
    }
    #[cfg(feature = "ml")]
    PdfError::Ml(reason) => {
      ("ML_ERROR", format!("ML error: {}", reason))
    }
    #[cfg(feature = "ocr")]
    PdfError::Ocr(reason) => {
      ("OCR_ERROR", format!("OCR error: {}", reason))
    }
    ...
    PdfError::RecursionLimitExceeded(limit) => {
      ("RECURSION_LIMIT_EXCEEDED", format!("Recursion depth limit exceeded (max: {})", limit))
    }
    PdfError::InvalidOperation(reason) => {
      ("INVALID_OPERATION", format!("Invalid operation: {}", reason))
    }
    PdfError::Barcode(reason) => {
      ("BARCODE_ERROR", format!("Barcode error: {}", reason))
    }
  };
