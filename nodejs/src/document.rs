...
use crate::metadata::{XMPMetadata, DocumentInfo, AcroForm, PageLabel, EmbeddedFile};
...
impl PdfDocument {
  ...
  pub fn open_from_bytes(data: napi::bindgen_prelude::Buffer) -> napi::Result<PdfDocument> {
    ...
    let doc = PdfDocumentImpl::open_from_bytes(&bytes).map_err(|e| crate::errors::map_error(e))?;
    Ok(PdfDocument { inner: doc })
  }
  ...
  pub fn get_page_count(&self) -> napi::Result<i32> {
    let count = self.inner.page_count().map_err(|e| crate::errors::map_error(e))?;
    Ok(count as i32)
  }
  ...
  pub fn extract_text(&self, page_index: i32) -> napi::Result<String> {
    let text = self.inner.extract_text(page_index as usize)
      .map_err(|e| crate::errors::map_error(e))?;
    Ok(text)
  }
  ...
  pub async fn extract_text_async(&self, page_index: i32) -> napi::Result<String> {
    ...
    tokio::task::spawn_blocking(move || {
      inner.extract_text(idx).map_err(|e| crate::errors::map_error(e))
    })
    .await
  ...
  pub fn to_markdown(
    ...
    let markdown = self.inner.to_markdown(page_index as usize)
      .map_err(|e| crate::errors::map_error(e))?;
    Ok(markdown)
  }
  ...
  pub async fn to_markdown_async(
    ...
    tokio::task::spawn_blocking(move || {
      inner.to_markdown(idx).map_err(|e| crate::errors::map_error(e))
    })
    .await
    .map_err(|e| napi::Error::new(napi::Status::GenericFailure, format!("Task error: {}", e)))?
  }
  ...
  pub fn to_markdown_all(&self, options: Option<ConversionOptions>) -> napi::Result<String> {
    let markdown = self.inner.to_markdown_all()
      .map_err(|e| crate::errors::map_error(e))?;
    Ok(markdown)
  }
  ...
  pub fn to_html(
    ...
    let html = self.inner.to_html(page_index as usize)
      .map_err(|e| crate::errors::map_error(e))?;
    Ok(html)
  }
  ...
  pub fn to_html_all(&self, options: Option<ConversionOptions>) -> napi::Result<String> {
    let html = self.inner.to_html_all()
      .map_err(|e| crate::errors::map_error(e))?;
    Ok(html)
  }
