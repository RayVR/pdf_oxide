use crate::dom::{AnnotationData, PageData};
use crate::elements::PdfText;
use crate::types::{AnnotationContent, ElementContent, Rect, SearchOptions, SearchResult};
...
impl PdfPage {
    ...
    pub fn new(page_index: usize) -> Self {
        let data = PageData::new(
            page_index, 612.0, // Default: US Letter width in points
...
impl PdfPage {
    ...
    pub fn find_text(
        ...
        let texts = if let Some(opts) = &options {
            if opts.case_sensitive.unwrap_or(false) {
                self.data
                    .find_text_by_predicate(|t| t.text.contains(&query))
            } else {
    ...
    pub fn add_annotation(&mut self, annotation: AnnotationContent) -> napi::Result<String> {
        ...
        let annotation_id =
            format!("annotation_{}_{}", self.data.page_index, self.data.annotations.len());
